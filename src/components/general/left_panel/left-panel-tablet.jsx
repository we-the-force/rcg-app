...
export default function LeftPanelTablet(props) {
    ...
    return (
        <Block className="left_panel_tablet">
            <Link onClick={(e) => { changeBackdropOpen(e, setSearchPop) }} className="icon-link search" iconMaterial="search" icon="search"></Link>
            <Link onClick={(e) => { changeBackdropOpen(e, setTVPop) }} className="icon-link tv"><img src={TVLight} alt="" /></Link>
            <Link onClick={(e) => { changeBackdropOpen(e, setRadioPop) }} className="icon-link radio_"><img src={radio} alt="" /></Link>
            ...
            <div className="links">
                <Link href="/derecho_replica" className="text-link derecho-replica">Derecho de réplica</Link>
            ...
            <Popup
                ...
                onPopupClose={(e) => { changeBackdropClose(setSearchPop, search_pop[1]) }}
            >
            ...
            <Popup
                ...
                opened={tv_pop[0]}
                style={{ top: tv_pop[1] }}
                onPopupClose={(e) => { changeBackdropClose(setTVPop, tv_pop[1]) }}
            >
                <List>
                    {
                        tv_channels.map((channel, key) => {
                            return (<ListItem key={key} link={`/tv/${channel.url}`} popupClose >{channel.nombre}</ListItem>)
                        })
                    }
                </List>
            </Popup>
            <Popup
                ...
                onPopupClose={(e) => { changeBackdropClose(setRadioPop, radio_pop[1]) }}
            >
                <List>
                    {
                        radio_stations.map((station, key) => {
                            return (<ListItem key={key} link={`/radio/${station.url}`} popupClose >{station.nombre}</ListItem>);
                        })
                    }
                </List>
            </Popup>
            <Popup
                ...
                onPopupClose={(e) => { changeBackdropClose(setMorePop, more_pop[1]) }}
            >
                <List>
                    {/* <ListItem popupClose link="#">Fundación RCG</ListItem> */}
                    <ListItem popupClose link="/espectaculares">Espectaculares</ListItem>
                    <ListItem popupClose link="/calca">Registra tu calca</ListItem>
                </List>
            </Popup>
            <Popup
            ...
            <Popup
                ...
                onPopupClose={(e) => { changeBackdropClose(setNosotrosPop, nosotros_pop[1]) }}
            >
                <List>
                    <ListItem popupClose link="/nosotros">Nosotros</ListItem>
                    <ListItem popupClose link="/contacto">Contacto</ListItem>
                </List>
            </Popup>
        </Block>
    );
