...
export default function Autor(props) {
    ...
    let numArticulosAutores = !loading && autoresArticulos.groupBy.autor.length > 0 ? autoresArticulos.groupBy.autor.map((val) => {
        return {
            autor: val.key,
    ...
    let centerPanel = loading ?
        ...
        <AutorPanel
            articulos={articulos}
            autor={autor}
            articulosNum={autorArticulos.groupBy.autor.length > 0 ?
                data.autorArticulos.groupBy.autor[0].connection.aggregate.count : 0
            }
        />;
